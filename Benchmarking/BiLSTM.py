...
model = Sequential()
model.add(Bidirectional(LSTM(n_neurons, activation='tanh', return_sequences=False), input_shape=(n_steps_in, n_features)))
model.add(Dense(n_steps_out))
model.compile(optimizer='adam', loss='huber', metrics=[tf.keras.metrics.MeanSquaredError(), tf.keras.metrics.MeanAbsoluteError()])
