...
class CifarClient(fl.client.NumPyClient):
    def __init__(self, model, x_train, y_train, x_test, y_test):  # TODO add validation data x_val, y_val
        self.model = model
    ...
    def fit(self, parameters, config):
        ...
        functionFile.set_model_params(self.model, parameters)
        # Get hyperparameters for this round
        # batch_size: int = config["batch_size"] # changed commented out
        # epochs: int = config["local_epochs"]
        # Train the model using hyperparameters from config
        self.model.fit(self.x_train, self.y_train)
        parameters_prime = functionFile.get_model_parameters(self.model)
        ...
        results = {
            "loss": 0.0,
            "mean_squared_error": mean_squared_error(self.y_test, yhat),
            "mean_absolute_error": mean_absolute_error(self.y_test, yhat)
    ...
    def evaluate(self, parameters, config):
        ...
        loss = 0.5
        yhat = self.model.predict(self.x_test)
        mse_metric = mean_squared_error(self.y_test, yhat)
        mae_metric = mean_absolute_error(self.y_test, yhat)
        num_examples_test = len(self.x_test)
        return loss, num_examples_test, {"mean_squared_error": mse_metric,
